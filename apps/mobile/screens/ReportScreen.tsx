...
import { CTAButton, TrendLineChart, FadeSlideIn, pressedStyle } from '@fuel/ui';
...
/**
 * IA 0001: Report is no longer a tab. Every competitor has exactly one "how am
 * I doing over time" destination; Fuel had two, and neither was complete alone
 * (Trends held the charts, Report held the verdict and the target proposal).
 * This renders the report CARDS only — Progress owns the scroller and the tab
 * bar now.
 */
export function ReportBody({ theme, vm, onConfirmDay, onAccept, onAdjust }: ReportBodyProps) {
  const r = vm.report;
